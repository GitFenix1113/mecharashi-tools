...
export interface TimedActivity {
  ...
  /**
   * 站方備註：**官方沒寫、但玩家需要知道**的補充。前台會顯示。
   *
   * 例：「角雕刮刮樂固定是出外觀的活動」—— 這種慣例官方公告從不寫，
   * 卻是讀者最想先知道的一句話。
   *
   * 三個文字欄位的分工（混用會讓資料再也分不出來源，這是本專案一貫的底線）：
   *   · description —— **官方寫的**活動規則，多半由公告解析器抄進來
   *   · editorNote  —— **我們寫的**補充，前台顯示，標示為「備註」
   *   · note        —— **給維護者的待辦**（缺什麼、去哪查），前台永不顯示
   */
  editorNote?: string
