 ...
 *
 * ⚠ 另注：新機甲一律**先建檔再補數值**（站上的佔位慣例），所以會出現「四部位齊全、
 *   但全部欄位（含軀幹出力）都是 0」的機甲。它解析得出來，但 `output` 會是 0 ——
 *   呼叫端要顯示「可用出力」時應把它當**數值未公布**處理，而不是渲染成一台
 *   什麼都裝不下的機甲。這種佔位不是髒資料，別把它修掉。
 *
 * ⚠ **今天全庫 0 台命中這條路**（2026-08-28 直讀正式庫：90 台的軀幹出力全部 > 0）。
 *   原註解舉的例子 `mech_090_美杜莎MK2` **已經補完數值了**（output 2605、weight 835、
 *   四格皆 Ⅱ型接口；`manual: true` 仍在）。
 *   ⇒ 這是一條**目前沒有活例子、但一定會再被走到**的路徑：下一台新機甲建檔的那天就會。
