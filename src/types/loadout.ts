...
export interface LoadoutDraft {
  ...
  /**
   * 潛能等級 0–5（PLAN-052-N C-1）。**欄位不存在＝滿潛 5**。
   *
   * 潛能提升＝重複抽到同一機師累積的養成軸，五階內容為
   * ①初始 AP +1 ②駕駛等級→大師Ⅱ **③天賦能力加強** ④駕駛等級→大師Ⅲ ⑤初始 AP +1，
   * 外加每階「所有屬性 +2%」。本站目前只消費第 ③ 階（`TALENT_BOOST_POTENTIAL`）。
   *
   * ⚠ **存等級而不是「天賦提升了沒有」的布林**：五階裡有四階不是天賦，布林**無法還原等級**，
   *   等到要算 AP 或屬性時就得再加一個欄位並**遷移所有已流出的分享碼**。
   *   今天多存 3 個 bit，換的是那次遷移不必發生。
   *
   * ⚠ **未設定 ＝ 滿潛**，與站上的機師資料一致 —— 那份資料本身就是滿潛快照
   *   （`ap.init − apBase.init = 2` 佔 54/89；`masterLevel` 存的「大師Ⅲ」是第 4 階的產物）。
   *   預設 0 會讓每一份既有的分享碼在解開後突然少掉三位機師的減重。
   *
   * ⚠ **放頂層、不放進 `EquipSet`**（同 `ndLevels` / `skills`）：潛能是**機師**的養成狀態，
   *   不隨形態變動。掛在 set 上會讓海莉絲的三個分頁各存一份潛能，
   *   而玩家改了其中一頁之後，另外兩頁的天賦敘述與總重會與這一頁不一致。
   *
   * ⚠ 不存 `null`，用「欄位不存在」表示未設定（同 `backpackId`）：三態撞上
   *   `stripUndefined`（firestoreCore.ts）會變成「一旦填了就再也清不掉」。
   */
