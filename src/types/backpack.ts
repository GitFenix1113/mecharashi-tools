...
export interface Backpack {
   ...
   *
   * 用陣列而非單一 skillId：當初（Phase A）實務長度恆為 0 或 1，開成陣列的理由是
   * 「改陣列的成本只是渲染側多一層 map，日後出現雙技能背包才不用再走一次型別遷移」。
   * **Phase F 兌現了這個預留**——98 筆 S+ 複合背包各掛兩支，全庫 181 筆的分佈是 1 或 2。
   *
   * ⚠ **順序有語意**（PLAN-043 Phase F）：
   *   · `[0]` ＝ 本體側（功能背包那支），完整生效。
   *   · `[1..]` ＝ 材料側（變體背包那支），**不帶底線加成**——官方複合技能文本裡
   *     「軀幹耐久值提升X%」只出現一次且來自本體側。前台由 `resolveBackpackSkills()`
   *     標成 `role: 'addon'` 並扣掉該句；細節與 97/98 逐字佐證見 `utils/backpackSkills.ts`。
   *   排序被打亂不會壞編譯、也不會壞畫面，只會讓底線加成扣錯邊，所以後台若日後開放
   *   拖曳排序，要把這條寫進提示。
   */
  skillIds: string[]
}
