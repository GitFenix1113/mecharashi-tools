...
test('機體故事的型號行不是活動標題，但仍是內文區塊的邊界', () => {
  // 官方為每台新機甲附一段背景設定文，標題是型號（「LTX-AB01君權」）。
  // 它三條標題判準全中（含「」、短、無句讀），於是被當成下一個活動的標題 ——
  // 自己落單成 unmatched，每出一台新機甲就多一行。
  //
  // 但「不是標題」不等於「不是邊界」：拆掉它的標題身分卻不截斷區塊的話，
  // 整段故事文會被收進上一個卡池的 description（實測 50 篇多出 100～350 字設定文）。
  const r = parseAnnouncement({
    title: '【版本前瞻】x',
    text: `機甲獲取海運活動
【特選】鐵幕法典 – S級中型機甲「君權」
➤活動內容：S級機甲「君權」購置概率提升
➤活動時間：2026/05/14 10:00:00 起
「LTX-AB01君權」
由拉尼亞凱亞公司為布呂希爾家族專門研發的中型指揮特化機甲`,
  })
  assert.equal(r.activities.length, 1, '型號行不該被當成第二個活動的標題')
  assert.equal(r.activities[0].extracted.name, '鐵幕法典')
  assert.equal(r.activities[0].extracted.description, undefined, '故事文不進 description')
  assert.deepEqual(r.unmatched, [], '型號行與故事正文都要被認領，不留在未認領清單')
})

test('機體故事：型號與正文同一行時，正文的「」不能改而浮上未認領', () => {
  // 實測 628／1129 是這種寫法。只認領型號行的話，下一行的設定名詞（「撕裂框架」）
  // 會改而落進 unmatched —— 那只是把噪音換個位置。
  const r = parseAnnouncement({
    title: '【版本前瞻】x',
    text: `機甲獲取海運活動
【特選】衝刺！撕裂！粉碎！ – S級輕型機甲「格萊楊」
➤活動內容：S級機甲「格萊楊」購置概率提升
➤活動時間：2024/05/30 10:00:00 起
「DD-117 格萊楊」，由多爾工業開發的輕型格鬥機兵，與黑曜石出品的多普勒是競爭關係。
該機軀幹前端安裝了多爾工業特製的「撕裂框架」輔助鎖定係統，使被鎖定的機兵保持在打擊範圍內。`,
  })
  assert.equal(r.activities.length, 1)
  assert.deepEqual(r.unmatched, [])
})

test('「【X】補充說明」以下整段是抽獎條文：不進 description，也不留在未認領', () => {
  // 實測 1751：blockEnd 停在「【跨域海運】補充說明」，其下 12 行沒有任何活動認領，
  // 含【】又不以數字起頭的 4 行就浮上未認領清單（第 4／5／6／8 組全出自這裡）。
  //
  // ⚠ 修法不能是「讓補充說明行不算標題候選」—— 那樣區塊不再截斷，整段條文流進
  //   description，而 blockLines 的 `概率|保障` 過濾接不住「60%**機率**為嵐質框體」
  //   （官方同篇混用「概率」與「機率」）。所以是「照樣截斷，但整段認領」。
  const r = parseAnnouncement({
    title: '【版本前瞻】x',
    text: `機甲獲取海運活動
【跨域海運】
➤活動內容：獲取嵐質框體，指定S級機甲即刻入庫！
➤活動時間：2026/07/09 10:00:00 起
【跨域海運】補充說明
【購置一次】時必會獲得1個隨機部件
對於每個獨立的【跨域海運】,首次S級整機或4個嵐質框體的保障購置次數為9,此後恒定為15
【跨域海運】中獲得的保障標記可與其他購置共用。
【跨域海運】中獲得S級時有60%機率為嵐質框體,若某次購置獲得了不為嵐質框體的S級部件,則下一次必為嵐質框體。`,
  })
  assert.equal(r.activities.length, 1)
  assert.equal(r.activities[0].extracted.name, '跨域海運')
  assert.equal(r.activities[0].extracted.description, undefined, '抽獎條文不是活動說明')
  assert.deepEqual(r.unmatched, [], '整段條文都要認領掉')
})

test('「認得但不產活動」的段落：標題認領了，內容也要認領', () => {
  // SECTIONS 裡 kind 為 null 的段落（特別提醒／注意事項／新增商品／領獎範例…）
  // 早就標明「這裡不產活動」，但先前只認領標題行本身，內容全數落進未認領清單。
  // 實測 1713 的 6 行商店上下架公告就是這樣來的（未認領整理的第 2／7／9 組）。
  const r = parseAnnouncement({
    title: '【版本前瞻】x',
    text: `限時活動
【環島物流節】
➤活動時間：2026/06/11 05:00 - 2026/06/18 04:59
活動說明:
活動期間，指揮官完成供貨網路委託所獲進度必定翻倍！
特別提醒
部分商店內部份商品將到期並更換為新商品，請注意購入期限。
【鬥技後勤中心】
機甲「金剛」相關部件與架上販售中的模組販售期間即將到期
新商品機甲「銀閃」相關部件與新一批販售模組將隨之上架。
【邊境自貿地】
機甲「馬爾薩斯」相關部件與架上販售中的模組販售期間即將到期`,
  })
  assert.equal(r.activities.length, 1, '商店不是活動')
  assert.equal(r.activities[0].extracted.name, '環島物流節')
  assert.deepEqual(r.unmatched, [], '整個「特別提醒」段落都要認領')
})

test('※※※特別提醒※※※：裝飾寫法一樣算段落標題', () => {
  // SECTIONS 是完全比對，接不住加星號的版本（實測 642 的段落標題就長這樣）。
  const r = parseAnnouncement({
    title: '【版本前瞻】x',
    text: `限時活動
【環島物流節】
