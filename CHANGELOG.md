...
- **simulator**: PLAN-052 使用者回饋批次 —— 機師層、模組一鍵裝滿、版面歸位 (`34dfd5a`)
- **admin**: 神經驅動算力與機甲接口自動帶入，不再逐格手打 (`f880e70`)
- **simulator**: 模組直接替換 —— 移除「這格已裝別顆」這個不存在的關卡 (`7f5ac6e`)
- **simulator**: PLAN-052-G 模組槽上線 —— 四個接口可配置、同族堆疊與超限提醒 (`2a69e5f`)
- **loadout**: PLAN-052-D Phase D 完成 — 元件進分享碼與匯出圖（15／15） (`08df357`)
- **loadout**: PLAN-052-D Phase C 完成 — 元件面板可操作（11／15） (`63a6d22`)
...
- **loadout**: 元件入口改成格子下方的窄列、面板加搜尋與篩選、拿掉機制解說 (`5a33466`)
- **loadout**: 元件入口補上槽位圖的 ⚙ 徽章 —— 站長找不到「怎麼設定元件」 (`f090e2b`)
- **loadout**: 配裝模擬器暫時收回內部測試 —— 入口四處一起收 (`94f9d9a`)
...
- **plan**: PLAN-052-G E-4 回寫總綱 —— 052-G 25/25 全數完成 (`cf47f8d`)
- **plan**: PLAN-052-F 形態獨立配裝完工歸檔（14/14），並回寫總綱 (`27726b0`)
- **plan**: 建立 PLAN-052-F 形態獨立配裝，並記錄曜的形態裁決 (`1fe3633`)
- **plan**: PLAN-052-G 決策三改為延後 —— 全站預設滿階，Σ moduleAddLevel 是常數 (`2da9421`)
- **plan**: 建立 PLAN-052-G 模組槽與部件混搭（22 工項，兩段式） (`159c429`)
- **plan**: PLAN-052-D 歸檔至歷史記錄（15 / 15 全數完成） (`631d354`)
- **plan**: PLAN-052-D 建檔 — 元件負載與互斥（15 工項） (`d0ec778`)
- **plan**: PLAN-052-J 歸檔至歷史記錄（8 / 8 全數完成） (`b4b015e`)
...
- **simulator**: 部件來源清單自己捲；分段控制改成看得出來可以按 (`cd6fcca`)
- **simulator**: 部位卡與裝備欄在寬螢幕下高度對齊 (`e6901c5`)
- **simulator**: 四部位的軀幹與腿部置中、元件縮圖放大 (`cb851d2`)
- **simulator**: 模組與元件改用縮圖呈現、四部位改十字排列、移除重複的背包入口 (`b3068ab`)
